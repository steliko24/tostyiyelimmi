...
var pythonic = "d1 = botlist"
if (pythonic = "d2 = botlist") {
}
...
module.exports.run = async(client,message,args)=> {
  ...
  let prompts = [
    "Hangi Rolü Verilmesini istiyorsanız kanala mesaj olarak atınız",
    "Hangi Tepkiye tıklayarak Rolleri Almalarını istiyorsanız emojiyi kanala mesaj olarak atınız",
    "Reaction Role için Ne yazılacak kanala yazınız",
    "Mesajın gideceği kanalı yazınız"
  ];
  let roles = await getResponses(message);
  let embed = new Discord.MessageEmbed()
    .setColor("#FF5349")
    .setFooter('d1')//pythonic
    .setDescription(
      `\`Rol:\`<@&${roles.role}>\n\`Emoji:\`${roles.emoji}\n\`Yazı:\`${roles.text}\n\`Kanal\`${roles.channel}`
    );
  let msg = await message.channel.send("Doğru Yaptığına Emin misin", embed);
  await msg.react("✅");
  await msg.react("❌");
  ...
  if (choice.emoji.name === "✅") {
    let emb = new Discord.MessageEmbed()//pythonic
      .setColor("#FF5349")
      .setDescription(roles.text || `Bu rolü almak için emojiye bas :  <@&${roles.role}> -D1`);
  ...
  let string = random(24);
    ...
    message.channel.send("Komut İptal Edildi - D1");
  }
